...
import { Bell, Clock, Calendar, UserCheck, Sparkles, AlertTriangle, Sun, Moon, Menu } from 'lucide-react';
...
export const TopHeader = ({ onOpenMobileMenu }) => {
  ...
  return (
    <header className="w-full glass-card rounded-none border-b border-slate-200/80 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl px-3 sm:px-6 py-2.5 sm:py-3 shadow-2xs sticky top-0 z-20 transition-colors">
      <div className="flex items-center justify-between gap-2 sm:gap-4">
        
        {/* Left: Mobile Menu Toggle + Section Title & Status Indicator */}
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div>
            <h1 className="text-sm sm:text-lg font-bold text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-2 truncate max-w-[180px] xs:max-w-[240px] sm:max-w-none">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">
              <span className="flex items-center gap-1.5 text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-2 py-0.5 rounded-full border border-emerald-200 dark:border-emerald-800 text-[10px] sm:text-[11px] font-semibold">
